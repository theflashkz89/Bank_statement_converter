...
> **文档状态**: ⏸️ 开发暂停（因更高优先级任务）  
> **更新日期**: 2025-01-XX  
> **当前版本**: V1.0（原生PDF解析）- 开发中  
> **未来版本**: V2.0（OCR扫描件支持）  
...
> **开发状态**: ⏸️ **暂停开发**（因更高优先级任务）  
> **最后更新**: 2025-01-XX  
> **当前阶段**: Stage 4 (HSBC解析器优化) - 部分完成

### 13.1 已完成功能 ✅

#### 13.1.1 HSBC 解析器三级解析机制
**完成时间**: 2025-01-XX

**实现内容**:
1. ✅ **Level 1 (正则解析)**: 保持现有正则/文本分割逻辑
2. ✅ **Level 2 (数学校验)**: 新增 `_validate_math()` 方法
   - 校验公式: `abs((prev_balance + credit - debit) - new_balance) < 0.01`
   - 允许误差: < 0.01
3. ✅ **Level 3 (AI 兜底)**: 新增 `_parse_line_with_ai()` 方法
   - 使用 DeepSeek API 解析错位交易文本
   - 自动提取 Debit、Credit、Balance、Payee
   - 完整的错误处理和降级机制

**代码位置**:
- `src/parsers/hsbc_parser.py`
  - `_validate_math()` (第323-337行)
  - `_parse_line_with_ai()` (第339-440行)
  - `_save_text_transaction()` (已集成三级机制，第442-568行)

**测试脚本**:
- `test_hsbc_ai_validation.py` - 完整的测试套件
  - 数学校验单元测试 ✅ (6/6 通过)
  - AI 解析功能测试 ✅ (3/3 通过)
  - 完整解析流程测试 ⚠️ (部分通过，发现余额不连续问题)

### 13.2 遇到的困难点 ⚠️

#### 困难点 1: AI 兜底触发机制不完善
**问题描述**:
- 测试中发现大量"数学校验失败"警告，但未看到"使用 AI 解析结果"的日志
- 说明 AI 兜底可能未被正确触发，或日志级别设置导致信息未显示

**可能原因**:
1. 日志级别设置问题（DEBUG/INFO 日志未显示）
2. AI 解析被调用但返回 None（应该有警告日志但未显示）
3. `need_ai_fallback` 标志设置逻辑可能有遗漏

**已采取的措施**:
- ✅ 添加了"触发 AI 兜底解析"的 INFO 级别日志
- ✅ 将部分 DEBUG 日志提升为 WARNING/INFO 级别
- ⚠️ 需要进一步验证 AI 是否被正确调用

**待解决问题**:
- [ ] 验证 AI 调用是否真正执行（添加更详细的调用日志）
- [ ] 检查 API Key 配置是否正确
- [ ] 优化 AI 调用条件判断逻辑

#### 困难点 2: 余额不连续问题
**问题描述**:
- 完整解析测试中发现 16 条余额不连续的交易
- 误差范围从 32.86 到 199,321.90 不等
- 部分交易可能是多币种切换导致的正常现象（如 B/F BALANCE）

**典型错误示例**:
```
索引 36: 2024-01-17
  期望: 200395.8 + 0.0 - 0.0 = 200395.8
  实际: 1073.9, 误差: 199321.90
  描述: POS MDC (17JAN24) CITY SUPER LIMITED
```

**可能原因**:
1. **多币种切换**: HSBC PDF 中不同币种账户的余额是独立的，切换时余额会跳跃
2. **Level 1 解析错误**: 正则提取的 Balance 本身就是错误的（可能是其他币种的余额）
3. **AI 解析未生效**: 虽然数学校验失败，但 AI 解析可能未成功修复

**待解决问题**:
- [ ] 区分"多币种切换导致的余额跳跃"（正常）和"解析错误导致的余额不连续"（异常）
- [ ] 优化 Level 1 正则提取逻辑，提高 Balance 识别准确率
- [ ] 验证 AI 解析是否能修复这些错误
- [ ] 添加币种切换检测，在切换时重置运行余额

#### 困难点 3: 性能考虑
**问题描述**:
- AI 解析是逐行调用的，如果大量交易触发 AI 兜底，可能导致：
  - 解析时间大幅增加（每次 API 调用约 1-2 秒）
  - API 调用费用增加
  - 可能触发 Rate Limit

**当前策略**:
- ✅ 只在"数学校验失败"或"正则提取失败"时才调用 AI（避免每行都调用）
- ⚠️ 但测试中发现大量数学校验失败，如果都调用 AI，性能影响较大

**待优化方向**:
- [ ] 考虑批量 AI 解析（将多条失败记录合并为一批）
- [ ] 添加 AI 调用频率限制（如每页最多调用 N 次）
- [ ] 优化 Level 1 解析逻辑，减少需要 AI 兜底的情况

### 13.3 待完成任务 📋

#### 高优先级
- [ ] **验证 AI 兜底机制**: 确认 AI 是否在数学校验失败时被正确调用
- [ ] **修复余额不连续问题**: 区分正常的多币种切换和解析错误
- [ ] **优化日志输出**: 确保关键信息（AI 调用、解析结果）能被正确记录

#### 中优先级
- [ ] **性能优化**: 考虑批量 AI 解析或调用频率限制
- [ ] **完善测试用例**: 添加更多边界情况测试（多币种切换、复杂交易等）
- [ ] **代码重构**: 优化 `_save_text_transaction` 方法的逻辑，提高可读性

#### 低优先级
- [ ] **文档完善**: 更新代码注释，说明三级解析机制的工作原理
- [ ] **错误处理增强**: 添加更详细的错误分类和处理策略

### 13.4 技术债务 💳

1. **日志系统**: 需要统一日志级别配置，确保关键信息可见
2. **测试覆盖**: 当前测试主要针对单行解析，缺少完整 PDF 解析的端到端测试
3. **代码注释**: 三级解析机制的复杂逻辑需要更详细的文档说明

### 13.5 恢复开发时的建议 🔄

**建议步骤**:
1. **第一步**: 运行 `test_hsbc_ai_validation.py`，查看当前状态
2. **第二步**: 检查日志文件，确认 AI 是否被调用
3. **第三步**: 针对余额不连续问题，先区分"正常切换"和"解析错误"
4. **第四步**: 优化 Level 1 解析逻辑，减少需要 AI 兜底的情况
5. **第五步**: 完善测试用例，确保修复后不会引入新问题

**关键文件**:
- `src/parsers/hsbc_parser.py` - 主要修改文件
- `test_hsbc_ai_validation.py` - 测试脚本
